#!/usr/bin/env python
"""Generate deterministic, balanced Grad-CAM panels for appendix reporting.

Samples are selected by a stable hash of fold/scenario/file name within each true
class, independently of model predictions. This prevents favorable-example cherry
picking and uses the same selected image for benchmark/proposed comparisons.
"""
...
def save_paired_comparison_panels(metadata: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    """Create one 2×3 benchmark/proposed comparison panel per selected input."""
    import matplotlib
