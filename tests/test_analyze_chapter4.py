...
class Chapter4InputValidationTests(unittest.TestCase):
    ...
    def test_rejects_validation_results(self) -> None:
    ...
    def test_rejects_results_without_split_or_legacy_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self._write_cases(root, evaluation_split=None)
            with self.assertRaisesRegex(ValueError, "legacy provenance"):
                load_cases(root, expected_folds=5)

    def test_accepts_verified_legacy_test_results(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self._write_cases(root, evaluation_split=None)
            for _, (model_type, scenario) in CASES.items():
                case_dir = root / f"{model_type}_{scenario}"
                results_path = case_dir / "all_fold_results.csv"
                frame = pd.read_csv(results_path)
                frame["fold_manifest_sha256"] = "manifest"
                frame["training_script_sha256"] = "script"
                frame.to_csv(results_path, index=False)
                for fold_index in range(1, 6):
                    fold_name = f"fold_{fold_index}"
                    pd.DataFrame(
                        {"filepath": [f"/project/data_5_fold/{fold_name}/test/normal/image.jpg"]}
                    ).to_csv(case_dir / f"{fold_name}_predictions.csv", index=False)
            combined = load_cases(root, expected_folds=5)
            self.assertEqual(len(combined), 20)
